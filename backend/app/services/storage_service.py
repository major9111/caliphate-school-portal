"""Storage service (Supabase or local fallback)."""
import os
import uuid
...
class StorageService:
    """Uploads files to Supabase Storage, or falls back to local disk."""
    ...
    async def upload(self, file: UploadFile, folder: str = "general") -> str:
        ...
        if self.use_supabase:
            ...
            self.client.storage.from_(settings.SUPABASE_BUCKET).upload(path, content, {"content-type": file.content_type or "application/octet-stream"})
            public = self.client.storage.from_(settings.SUPABASE_BUCKET).get_public_url(path)
            return public
    ...
    async def delete(self, url: str) -> None:
        if self.use_supabase and settings.SUPABASE_BUCKET in url:
            try:
