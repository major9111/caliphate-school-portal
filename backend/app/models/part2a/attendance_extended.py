"""Enhanced attendance models with approval workflow."""
...
class AttendanceRegister(TimestampMixin, Base):
    """Daily attendance register (one per class per day)."""
    __tablename__ = "attendance_registers"

    id = Column(Integer, primary_key=True, index=True)
    class_level_id = Column(Integer, ForeignKey("class_levels.id"), nullable=False)
    ...
    period = Column(String(20), default="morning")  # morning, afternoon, full_day
    status = Column(String(20), default="draft")  # draft, submitted, approved, corrected
    ...
    __table_args__ = (
        Index("ix_register_unique", "class_level_id", "arm_id", "date", "period", unique=True),
        Index("ix_register_date", "date", "status"),
    )


class AttendanceRecord(TimestampMixin, Base):
    """Individual attendance record per student per register."""
    __tablename__ = "attendance_records"

    id = Column(Integer, primary_key=True, index=True)
    register_id = Column(Integer, ForeignKey("attendance_registers.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    __table_args__ = (
        Index("ix_record_register_student", "register_id", "student_id", unique=True),
    )
