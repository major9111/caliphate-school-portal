"""Academic sessions, terms, classes, subjects, timetables."""
from sqlalchemy import Column, Integer, String, ForeignKey, Boolean, Text, Time, Index
...
class AcademicTerm(TimestampMixin, Base):
    ...
    name = Column(String(30), nullable=False)  # First Term, Second Term, Third Term
    start_date = Column(String(20), nullable=False)
    end_date = Column(String(20), nullable=False)
    is_current = Column(Integer, default=0)
...
class ClassLevel(TimestampMixin, Base):
    __tablename__ = "class_levels"

    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(50), nullable=False)  # Nursery 1, Primary 5, JSS 2, SS 3
    section = Column(String(30), nullable=False)  # nursery, primary, junior_secondary, senior_secondary
    order_index = Column(Integer, default=0)
    capacity = Column(Integer, default=40)
    is_active = Column(Integer, default=1)

    subjects = relationship("Subject", back_populates="class_level")


class Section(TimestampMixin, Base):
    """Parallel sections within a class (A, B, C)."""
...
class SubjectAssignment(TimestampMixin, Base):
    """Assigns a teacher to a subject for a class/term."""
...
class Timetable(TimestampMixin, Base):
    ...
    section_id = Column(Integer, ForeignKey("sections.id"), nullable=True)
    term_id = Column(Integer, ForeignKey("academic_terms.id"), nullable=False)
    day_of_week = Column(String(10), nullable=False)  # Monday..Friday/Saturday
