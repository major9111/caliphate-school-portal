...
class StudentRepository(BaseRepository[Student]):
    ...
    def get_with_guardian(self, id: int) -> Optional[Student]:
        return (
    ...
    def get_many_with_guardian(self, skip: int = 0, limit: int = 20, search: Optional[str] = None) -> List[Student]:
        q = self.db.query(Student).options(joinedload(Student.guardian))
        if search:
            like = f"%{search}%"
            q = q.filter(
    ...
    def count_by_gender(self) -> dict:
        ...
        rows = self.db.query(Student.gender, func.count(Student.id)).group_by(Student.gender).all()
        return {g: c for g, c in rows}

    def count_active(self) -> int:
        return self.db.query(Student).filter(Student.is_active == 1).count()
